package com.example.pasir_lipior_michal.service;

import com.example.pasir_lipior_michal.dto.DebtDTO;
import com.example.pasir_lipior_michal.model.Debt;
import com.example.pasir_lipior_michal.model.Group;
import com.example.pasir_lipior_michal.model.Transaction;
import com.example.pasir_lipior_michal.model.User;
import com.example.pasir_lipior_michal.model.TransactionType; // <--- DODAJ IMPORT DLA TransactionType
import com.example.pasir_lipior_michal.repository.DebtRepository;
import com.example.pasir_lipior_michal.repository.GroupRepository;
import com.example.pasir_lipior_michal.repository.TransactionRepository; // <--- DODAJ IMPORT DLA TransactionRepository
import com.example.pasir_lipior_michal.repository.UserRepository;
import jakarta.persistence.EntityNotFoundException;
// Usunięto nieużywane importy Argument i MutationMapping, bo to jest Service, a nie Controller GraphQL
import org.springframework.stereotype.Service;

import java.util.List;

@Service
public class DebtService {

    private final DebtRepository debtRepository;
    private final GroupRepository groupRepository;
    private final UserRepository userRepository;
    private final TransactionRepository transactionRepository; // <--- ZADEKLARUJ POLE

    // ZAKTUALIZUJ KONSTRUKTOR
    public DebtService(DebtRepository debtRepository,
                       GroupRepository groupRepository,
                       UserRepository userRepository,
                       TransactionRepository transactionRepository) { // <--- DODAJ DO KONSTRUKTORA
        this.debtRepository = debtRepository;
        this.groupRepository = groupRepository;
        this.userRepository = userRepository;
        this.transactionRepository = transactionRepository; // <--- PRZYPISZ W KONSTRUKTORZE
    }

    public List<Debt> getGroupDebts(Long groupId) {
        return debtRepository.findByGroupId(groupId);
    }

    public void deleteDebt(Long debtId, User currentUser) {
        Debt debt = debtRepository.findById(debtId)
                .orElseThrow(() -> new EntityNotFoundException("Dług o ID " + debtId + " nie istnieje."));

        if (!debt.getCreditor().getId().equals(currentUser.getId())) {
            throw new SecurityException("Tylko wierzyciel może usunąć ten dług.");
        }

        debtRepository.delete(debt);
    }

    public Debt createDebt(DebtDTO debtDTO) {
        Group group = groupRepository.findById(debtDTO.getGroupId())
                .orElseThrow(() -> new EntityNotFoundException("Nie znaleziono grupy o ID: " + debtDTO.getGroupId()));
        User debtor = userRepository.findById(debtDTO.getDebtorId())
                .orElseThrow(() -> new EntityNotFoundException("Nie znaleziono dłużnika o ID: " + debtDTO.getDebtorId()));
        User creditor = userRepository.findById(debtDTO.getCreditorId())
                .orElseThrow(() -> new EntityNotFoundException("Nie znaleziono wierzyciela o ID: " + debtDTO.getCreditorId()));

        Debt debt = new Debt();
        debt.setGroup(group);
        debt.setDebtor(debtor);
        debt.setCreditor(creditor);
        debt.setAmount(debtDTO.getAmount());
        debt.setTitle(debtDTO.getTitle());

        return debtRepository.save(debt);
    }

    public boolean markAsPaid(Long debtId, User user) {
        Debt debt = debtRepository.findById(debtId)
                .orElseThrow(() -> new EntityNotFoundException("Nie znaleziono długu"));

        if (!debt.getDebtor().getId().equals(user.getId())) {
            throw new SecurityException("Nie jesteś dłużnikiem");
        }

        debt.setMarkedAsPaid(true);
        debtRepository.save(debt);
        return true;
    }

    public boolean confirmPayment(Long debtId, User user) {
        Debt debt = debtRepository.findById(debtId)
                .orElseThrow(() -> new EntityNotFoundException("Nie znaleziono długu"));

        if (!debt.getCreditor().getId().equals(user.getId())) {
            throw new SecurityException("Nie jesteś wierzycielem");
        }

        if (!debt.isMarkedAsPaid()) {
            throw new IllegalStateException("Dłużnik jeszcze nie oznaczył jako opłacone");
        }

        debt.setConfirmedByCreditor(true);
        debtRepository.save(debt);

        // + Dodanie transakcji dla wierzyciela (przychód)
        // Upewnij się, że konstruktor Transaction przyjmuje te argumenty
        // Jeśli Twój TransactionService ma metodę createTransaction, lepiej jej użyć dla spójności
        Transaction incomeTx = new Transaction();
        incomeTx.setAmount(debt.getAmount());
        incomeTx.setType(TransactionType.INCOME); // <--- TERAZ POWINNO DZIAŁAĆ
        incomeTx.setTags("Spłata długu"); // Użyj pola 'tags' jeśli istnieje lub 'notes'
        incomeTx.setNotes("Spłata długu od: " + debt.getDebtor().getEmail());
        incomeTx.setUser(debt.getCreditor());
        // transaction.setTimestamp(LocalDateTime.now()); // Jeśli TransactionService to ustawia, to nie musisz tu
        transactionRepository.save(incomeTx); // <--- TERAZ POWINNO DZIAŁAĆ

        // - Dodanie transakcji dla dłużnika (wydatek)
        Transaction expenseTx = new Transaction();
        expenseTx.setAmount(debt.getAmount());
        expenseTx.setType(TransactionType.EXPENSE); // <--- TERAZ POWINNO DZIAŁAĆ
        expenseTx.setTags("Spłata długu");
        expenseTx.setNotes("Spłacono dług dla: " + debt.getCreditor().getEmail());
        expenseTx.setUser(debt.getDebtor());
        // expenseTx.setTimestamp(LocalDateTime.now());
        transactionRepository.save(expenseTx); // <--- TERAZ POWINNO DZIAŁAĆ

        return true;
    }
}