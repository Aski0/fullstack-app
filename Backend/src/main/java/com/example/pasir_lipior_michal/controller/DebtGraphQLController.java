...
import com.example.pasir_lipior_michal.service.DebtService;
import com.example.pasir_lipior_michal.service.TransactionService; // <--- DODAJ IMPORT
...
public class DebtGraphQLController {
    ...
    private final TransactionService transactionService; // <--- ZADEKLARUJ POLE

    // ZAKTUALIZUJ KONSTRUKTOR
    public DebtGraphQLController(DebtService debtService, TransactionService transactionService) {
        this.debtService = debtService;
        this.transactionService = transactionService; // <--- PRZYPISZ W KONSTRUKTORZE
    }
    ...
    public Boolean deleteDebt(@Argument Long debtId) {
        User currentUser = transactionService.getCurrentUser(); // <--- TERAZ POWINNO DZIAŁAĆ
